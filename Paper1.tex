...
For larger hybridizations the shallow peak suggests a suppression of impact ionization with respect to direct photovoltaic excitations. Acoustic phonons slightly enhance the photocurrent for small driving frequencies and suppress it at frequencies around the main peak 
% This effect occurs at all considered hybridization strengths.
 at all considered hybridization strengths.
...
For this goal we consider the simplified model shown in Fig.~\ref{fig:setup}, consisting of a Hubbard layer located between two leads. The leads have themselves a wide band, as expected for good condutors, which are effectively narrowed due to the weak coupling to intermediate layers which we take as simplified models for the contacts. Electrons on the Hubbard layer interact locally with acoustic phonons. An external periodic electric field induces a Floquet steady state with a current flowing from the lead with the lower to the higher chemical potential, thus transferring its energy.
...
Phonons are 
%The local e-ph coupling is
 included as in 
%EACOMM check, put also previous references
Ref.~\cite{ma.ga.22u} in the form of  acoustic  branches attached to each lattice site and coupled to the electrons via the Hamiltonian 
\begin{equation}\label{eq:e-ph_ham} 
\hat{H}_{\text{e-ph}} = g \sum_{i\sigma} \hat{n}_{i\sigma} \hat{x}_{i},  
\end{equation}
with $g$ as e-ph coupling and $\hat{x}_{i} \equiv \frac{1}{\sqrt{2}} \left( \hat{b}^{\dagger}_{i} + \hat{b}_i \right)$, where $\hat{b}^{\dagger}_{i}$ ($\hat{b}_{i}$) creates (annihilates) a phonon of the acoustic branch at site $i$, with dispersion relation described by $\hat{H}_{\text{ph}}$~\cite{ma.ga.22u}, see below.  
In contrast to Holstein phonons these have a finite bandwidth which is more effective in energy dissipation.
Details on the leads are provided in~\ref{sec:Dyson_equation}.  
 
For mathematical simplicity~\cite{ge.ko.92,ao.ts.14}, 
 we consider the correlated central region as a $d$-dimensional layer and consider the limit of infinite dimensions $d \rightarrow \infty$ by rescaling the hopping as 
%EACOMM check parenthesis
$t_{\text{c}}=t^{\ast}/(2\sqrt{d})$. Taking $a=1$, sums over the crystal momentum ${\bf k}$ of a generic quantity $\chi$ read $\sum_{{\bf k}} \chi(\omega,{\bf k}) \rightarrow \int d\epsilon \int   d\overline{\epsilon} \ \rho(\epsilon,\overline{\epsilon}) \chi(\omega;\epsilon,\overline{\epsilon})$ with $\rho(\epsilon,\overline{\epsilon}) = (1/\pi t^{\ast 2}) \exp[-( \epsilon^{2} + \overline{\epsilon}^{2})/t^{\ast 2}]$ as the joint density of states~\cite{ts.ok.08} and with
% 
...
% In Tommasos paper we also chose -q=1. Given the amplitude of the vector potential above, setting q=-1 allows us to take positive values of E_0 and Omega. same sign of j0 (see old draft). increasing the mu I have negative current meaning that particle/electric current is flowing in the opposite direction. 
...
\end{equation}
where in our approximation the electron-electron $ \underline{{\bf \Sigma}}(\omega_n;\epsilon,\overline{\epsilon})$ and electron-phonon $\underline{{\bf \Sigma}}_{\text{e-ph}}(\omega_n;\epsilon,\overline{\epsilon})$ self energies contribute independently to the total self energy~\footnote{In fact there is an indirect feedback via the DMFT self-consistency.}
...
The electron self-energy (SE) $\underline{\bf{\Sigma}}$ is obtained from F-DMFT and therefore, in this approximation, independent of the crystal momentum, i.e. $\underline{\bf{\Sigma}}(\omega;\epsilon,\overline{\epsilon}) \simeq \underline{\bf{\Sigma}}(\omega)$. Further details are given in Sec.~\ref{sec:FDMFT_implementation}. 
In the same spirit, the e-ph SE is included locally as $\underline{\bf{\Sigma}}_{\text{e-ph}}(\omega;\epsilon,\overline{\epsilon}) \simeq \underline{\bf{\Sigma}}_{\text{e-ph}}(\omega)$. In terms of the Keldysh contour time arguments $z$ and $z^\prime$, it has the form 
...
For completeness, we now
 briefly describe now the self-consistent F-DMFT scheme used.
...
%EACOMM I eliminated this and the next subsubsection, is this okay?  
%\subsubsection{Floquet-diagonal self-energy approximation}
%\label{sec:FDSA} 
 
%EACOMM check this paragraph, which I changed a bit
Eq.~\eqref{eq:imp_Dyson_eq} gives the bath hybridization function $\kel{\mat\Delta}(\omega)$ whose periodic time-dependence is encoded in its Floquet structure. 
This means that, in principle, one would have to solve a time-periodic impurity problem leading to a non-diagonal self-energy in general. However, in 
 Ref.~\cite{so.do.18} it was argued that for the electric field strengths considered, non-diagonal terms in the Floquet SE $\kel{\mat{\Sigma}}(\omega)$  can be safely neglected with respect to the diagonal ones. We follow this argument and adopt here 
this Floquet-diagonal self-energy approximation (FDSA) as well.
 In addition, it turns out that also for the 
 phonon SE $\underline{\bf{\Sigma}}_{\text{e-ph}}(\omega)$ this approximation is justified\footnote{More specifically, we have verified that in the parameters range in which FDSA is justified for the electron SE $\kel{\mat{\Sigma}}(\omega)$ (Sec.~\ref{sec:results}), the off-diagonal terms of the phonon SE $\underline{\Sigma}_{mm,\text{e-ph}}(\omega)$ are orders of magnitude smaller than the diagonal ones}. 
Consequently, we  have to solve a nonequilibrium stationary impurity problem, by using only the  $(0,0)$-Floquet matrix elements of $\kel{\mat{\Sigma}}(\omega)$ and $\kel{\mat\Delta}(\omega)$, 
%Eq.~\eqref{eq:imp_Dyson_eq}, 
as the other diagonal components can be reconstructed by using the property $\kel{\Sigma}_{mm}(\omega) = \kel{\Sigma}_{00}(\omega+m\Omega)$. 
...
%We now
% briefly summarize the impurity solver used in step (v) of the DMFT self-consistent loop %described in Sec.~\ref{sec:FDMFT_implementation}. 
In order to solve the electron-electron many-body problem (step (v) of the DMFT self-consistent loop),
 we use the auxiliary master equation approach (AMEA)~\cite{ar.kn.13,do.nu.14,do.ga.15,do.so.17}, which we now briefly summarize. 
Therein, we map the impurity problem onto an auxiliary open quantum system (AOQS) consisting of a finite number of bath sites $N_{\text{B}}$ attached to Markovian reservoirs described by the Lindblad equation. The  hybridization function $\kel{\Delta}_{\text{aux}}(\omega)$ of this AOQS is obtained by fitting the original DMFT one. The key point is that the many-body problem of this AOQS can be solved exactly 
...
All simulations are carried out with the dimensionless factor $\alpha\equiv t_{r}E_0/\Omega^{2}<0.5$ for which the FDSA is justified~\footnote{In Ref.~\cite{so.do.18} the factor is defined as $\alpha=t_{c}E_0/\Omega^{2}$ and $t_{\text{c}}=1$. Setting $t_{r}=1$, we define for consistency $\alpha$ as in the main text.}.
% and first-order photon absorption processes are dominant.
 Whenever the electron-phonon interaction is included, we take $g=0.8$ and unless stated otherwise $\omega_{\text{ph}}=0.1$.
...
\caption{(Color online) Schematic representation of the processes occuring in the model considered. Red bands on the left and right describe the leads' DOS, where the dark color highlights occupied states and the light color empty states. Green bands are the LHB and UHB of the central layer. Due to the hybridization with the metallic leads only a pseudogap $\Delta_{\text{pg}}$ is present between the bands. 
 The vertical axis represents the energy. Panel (a) sketches the quantities introduced at the beginning of Sec.~\ref{sec:results}. Panel (b) illustrates a direct excitation process, in which an electron is excited by a photon with energy $\Omega$ (yellow arrow) to the UHB and escapes into the right lead (red arrow). Panel (c) displays an II process, where the photoexcited electron in the UHB excites a second electron from LHB to UHB (black arrows) and both escape into the right lead. In the nonequilibrium steady state considered here, only processes recovering the initial configuration are allowed.} 
...
In comparison with Ref.~\cite{so.do.18} here the wide-band leads more realistically described a metallic structure. This makes the identification of II processes more subtle, as discussed below.
As in Ref.~\cite{so.do.18} we bypass transient behavior and consider directly the nonequilibrium Floquet steady state. In our analysis, we neglect higher-order processes such as multiparticle scattering processes. 
...
only a pseudogap $\Delta_{\text{pg}}$ is present between the LHB and the UHB.} 
size $\Delta_{\text{pg}}$. Only in this case, the photoexcited electron can acquire enough energy to excite a second one across the pseudogap. 
...
we expect a strong suppression of the current
we do not expect any current if the DOS of the correlated layer has a true gap~\footnote{The remaining leak current can be either produced by multiple-photon absorption processes, which however, are strongly suppressed, or by the fact that there is no true gap}.
%To overcome a true gap with $\Omega<\Delta_{\text{pg}}$  should take place, but as written in the main text, such effects are negligible in the studied parameters range.}. 

%~\footnote{To overcome a true gap with $\Omega<\Delta_{\text{pg}}$ multiple-photon absorption processes should take place, but as written in the main text, such effects are negligible in the studied parameters range.}. 
%If $\Delta_{\text{pg}}$ is a partial gap, we expect a suppression of the current.
...
This is in contrast to the case of Ref.~\cite{so.do.18} in which the peculiar ad-hoc narrow lead bands only allowed for II current in a certain $\Omega$ regime.
For this reason, and due to the fact that
%We want to stress, that 
the boundaries of these energy ranges are not strict, it is more tricky to disentangle  these physical processes in the present case.
...
We start with the case without e-ph interaction and address the behavior 
%investigate the dependence 
of  physical quantities as a function of $\Omega$ for different strengths of the lead-layer hybridization.
...
\begin{figure}[b]  
% \captionsetup{width=\textwidth} 
\includegraphics[width=\linewidth]{./figures_Paper1/j_vs_omega_mu1_v_0.4_0.8_e_E0_3_4.pdf}
\caption{(Color online) (a) Time-averaged steady-state current $j$ and (b) double occupancy $N_{\text{D}}$ as a function of $\Omega$, for $v=0.4,0.8$ at electric field amplitudes $E_0=3,4$ for the EO system. Default parameters are specified at the beginning of Sec.~\ref{sec:results}.} 
\label{fig:j_vs_omega_mu1_v_0.4_0.8_e_E0_3_4} 
\end{figure}

For $\Omega\gtrsim8$, the current increases further until it reaches its peak at $\Omega\approx 11$.
In this range both DE and II can occur in principle. The height of the peak is approximately the same for both $v$, despite the lead-layer hybridizations differing by a factor of two. The double occupation in this range shows a similar behavior as the current for $v=0.4$ with a peak around $\Omega\approx 11$, while it exhibits a plateau for $v=0.8$.
For $v=0.4$ the current curve is characterized by a substantial increase in slope around 
%EACOMM i put 7, this is where it actually occur
$\Omega \approx 7$, which cannot be observed for $v=0.8$.
 This, together with the comparable magnitude of the maxima at $\Omega\approx 11$ and the behavior of the double occupation strongly suggest that for $v=0.4$ a substantial rate of II processes take place, while these are absent or negligible for $v=0.8$.
...
Increasing $\Omega$ further produces excitations near the border of the UHB where the DOS is reduced, and, thus  current and double occupation decrease~\footnote{The current $j$ does not approach zero as expected for $\Omega>20$ because of the background current as discussed in Ref.~\cite{so.do.18}.}.


NEW PART: If the electric field amplitude $E_0$ is increased, as shown in Fig.~\ref{fig:j_vs_omega_mu1_v_0.8_e_E0_3_4}(a), the current curve develops an increase in slope around $\Omega\approx7$ and the double occupation (cf. Fig.~\ref{fig:j_vs_omega_mu1_v_0.8_e_E0_3_4}(b)) a peak around $\Omega\approx 11$ at the lead-hybridization value $v=0.8$. Such behavior suggests that at larger $E_0$ larger $v$ allows for II, although its contribution on the current is significantly suppressed than with smaller $v$ values. 

WHICH FIGURE I PUT? Fig.~\ref{fig:j_vs_omega_mu1_v_0.8_e_E0_3_4} or Fig.~\ref{fig:j_vs_omega_mu1_v_0.4_0.8_e_E0_3_4}? Fig.~\ref{fig:j_vs_omega_mu1_v_0.4_0.8_e_E0_3_4} confirms findings of $E_0=2$ case and it's in line with the message of the article (For a small hybridization to the leads, we obtain a peak in the photocurrent as a function of the driving frequency which can be associated with impact ionization processes.
For larger hybridizations the shallow peak suggests a suppression of impact ionization with respect to direct photovoltaic excitations.). In case we keep Fig.~\ref{fig:j_vs_omega_mu1_v_0.4_0.8_e_E0_3_4} do I have to comment the curves for $v=0.4$? do we have to add something to the abstract/introduction about this new part?
...
With a smaller pseudogap and more states at its edges, more electrons can be directly excited with a given driving frequency thereby increasing the photocurrent.
%%EACOMM, see other references for theese in gap states
~\footnote{This effect is loosely related to enhancement of current due to the phonon-induced in-gap ladder states in~\cite{ha.ar.22u}.}
 On the other hand, the suppression of the current by e-ph interaction in the range $7\lesssim\Omega\lesssim14$ is due to the dissipation-induced reduction of spectral weight at frequencies $|\omega| \gtrsim 3.5$, cf. Fig. \ref{fig:spec_filling_mu1_v_0.4_0.8_O_11_eph}. 
...
Summarizing, phonons enhance the current for almost all driving frequencies $\Omega\lesssim 7$ for which only DEs take place. They suppress the current in the high-frequency range $7\lesssim\Omega\lesssim14$.
%EACOMM I removed this sentence because there is no real causal relation since the current is suppressed in both cases.
%where II is the dominant electronic scattering process for $v=0.4$ and DE for $v=0.8$.  
%% By reducing the soft cutoff frequency, the spetral function of the phonons at low frequencies is increased. The idea is that this increases the influence of the phonons on the current thus increasing the current at low driving frequencies (DE) and reducing the current at driving frequencies around the peak (II). 
% 

%In order to verify the robustness of these results on the properties of the phonons, 
%we have  
This behavior is qualitatively valid for different values of 
the soft cutoff phonon frequency
 $\omega_{\text{ph}}$. This can be seen in
%is confirmed by
 Fig.~\ref{fig:j_vs_omega_mu1_v_0.4_0.8_E0_2_sweep_omegaph}, which shows that the impact of acoustic phonons on 
...
The results above suggest that the influence of acoustic phonons on the electronic scattering processes does not depend significantly on $v$, i.e. on whether II processes are significant or not. In other words, the effects of phonons does not depend significantly
on the rate at which carriers are injected into and removed from the layer.
 Fig.~\ref{fig:j_vs_omega_mu1_sweep_v_E0_2_eph} confirms this behavior for intermediate lead-layer hybridizations $v=0.5,0.6,0.7$.
%EACOMM I removed this sentence. It's not quite clear 
%%for which DE and II at $\Omega\gtrsim8$ are even stronger mixed and harder to separate.
 Also the location of the crossing in the current  between the EO and e-ph case is essentially independent of $v$, as can be seen in 
...
coincides with the current suppression and is probably due to a 
suppression of the mobility 
%stronger localization 
of photoexcited electrons in the UHB.
...
We investigated the influence of fermion leads and phonon dissipation on electron transport and spectral properties of a Mott insulating layer driven to the nonequilibrium steady state by an external periodic field. In order to realize a Mott-based photovoltaic setup between metallic leads, we considered a correlated layer coupled to acoustic phonons, located between wide-band fermion leads. 
We studied the influence of the strength 
of the coupling to the leads
on 
scattering process occurring at different driving frequencies and observed how the dissipation by acoustic phonons influences the photocurrent and the double occupation. We found evidence of a significant rate of II processes leading to a photocurrent peak 
%the dominant electron scattering mechanism responsible for the photocurrent peak 
at small hybridizations, while these are suppressed  for larger hybridizations. Dissipation via acoustic phonons slightly boosts the photocurrent for small driving frequencies and suppresses it at larger ones in the vicinity of the main peak. The effect of phonons is not affected by changes in the lead-layer hybridization.
...
Experimentally, the electric field amplitude considered in this paper~\footnote{We do not consider smaller electric field amplitudes $E_0$ since DMFT only converges slowly for such a choice of parameters.} is several orders of magnitude larger than that of the sunlight, so that these results are rather relevant for photoexcitations produced by intense laser pulses (see also the discussion in ~\cite{mu.we.18} rather than true photovoltaic systems. On the other hand, one may try to estrapolate the idea of 
%%EACOMM IMPORTANT fill and relate to the discussion larger e fields
in the other, namely to smaller electric fields for which a smaller hybridisation is necessary in order to support impact ionisation. For a more realistic system to model
oxide heterostructures~\cite{as.bl.13,pe.be.19}
the regime of small hybridisation should be replaced by a multilayer device in which photoexcited carrier, separated by an electric field gradient, have more time to induce impact ionisation processes. Also further effects should be taken into account, such as multiorbital correlated systems as well as impurity scattering. This will be the content of future studies.




%Moreover, in order to address the occurrence of II in realistic photovoltaic setups, one should extend such simplified model to multiorbital systems \cite{pe.be.19} in a multilayer setup, to model  where the correlated region consists of multiple layers and photoexcited carriers are separated via an electric field gradient. Another step forward would be to take into account the effect of impurity scattering and the feedback of the electrons onto the phonon dynamics with a self-consistent treatment.
