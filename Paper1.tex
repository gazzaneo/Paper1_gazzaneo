...
We study a model for photovoltaic energy collection consisting of a Mott insulating layer in presence of acoustic phonons, coupled to two wide-band fermion leads at different chemical potentials and driven %PGCH[
into a nonequilibrium steady state
%PGCH]
by a periodic electric field. We treat electron correlations with nonequilibrium dynamical mean-field theory (DMFT) using the so-called auxiliary master equation approach as impurity solver and include dissipation by acoustic phonons via the Migdal approximation. For a small hybridization to the leads, we obtain a peak in the photocurrent as a function of the driving frequency which can be associated with impact ionization processes.
...
%PGCH[ I changed some words and rephrased the last sentence in this paragraph after Enrico modified it
...
%PGCH: I put this section as a subsection as before  
...
%PGCH: REMOVED:the lead-layer hybridizations differing by a factor of two. ALTERNATIVE FORMULATION:
%PGCH[
...
%PGCH[ Added Enrico's part. I changed some words. 
...
This reduction of the pseudogap explains why the current in Fig.~\ref{fig:j_vs_omega_mu1_v_0.4_0.8_eph}(a) is slightly larger in presence of phonons for $\Omega\lesssim7$. %PGCH: removed reference to in gap phonon states
...
%PGCH: I find this sentence below not clearly motivated. In the e-ph case, the correlation between the increase of N_D and the current suppression is true, but it's also true that the double occupation is larger than the EO counterpart also when the current with phonons is enhanced in the low-frequency range.   
...
%PGCH: Removed the paragraph related to suggestions and possible experimental realization of our setup. 

%PGCH[ Added Enrico's part but I changed some words and rephrased
...
%PGCH: REMOVED: is. ALTERNATIVE FORMULATION:
%PGCH[
would be
%PGCH]
